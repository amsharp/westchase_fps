...
**Two more garages sit downtown** (around 2525,1712 and 2554,2406), each with its
east + west entrances wired into the street grid by short access roads.

## Downtown (the new eastern city)

The expansion map's eastern district is a full **downtown**: eight bank
skyscrapers cluster at the center, and ~780 mid-rise **filler buildings**
(brick rowhouses, storefront businesses, glass offices, and a highrise
apartment) line every street on both sides, fronts facing the road. Buildings
get **taller toward the skyscraper core** and shorter out on the fringes;
they're packed into contiguous "walls" in places and spaced out for alleyways
in others, with a couple of parking lots and the two parking garages filling
block interiors. The building models live in `citybuildings.js` (`CITY_BUILDINGS`
catalog, pushed into `MODEL_CATALOGS`); placements are generated by
`tools/citygen/placebuildings.js` (frontage-walks the downtown box, tiers by
distance from the skyscraper cluster) and baked into `REMAP_MODELS` /
`westchase_map.json`.

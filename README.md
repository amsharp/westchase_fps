...
the cuffs. The taser can't touch you in a car, in the air, or up on a roof.
...
You can now **drop more garages anywhere from the map editor** — pick the
"Parking Garage" building, place/move/delete it like any building; it bakes into
`REMAP_GARAGES` and the game builds it there. (Wire its east + west entrances to
the street grid with short roads.)

## Downtown (the new eastern city)

The expansion map's eastern district is a full **downtown**: eight bank
skyscrapers cluster at the center, and ~500 **filler buildings** (brick
rowhouses, storefront businesses, big glass offices, and towering highrise
apartments) line every street on both sides, **fronts facing the road**.
Buildings get **taller toward the skyscraper core** — offices and near-skyscraper
...
rubble event. When a tower comes down it **flattens its neighbours**: every
nearby building (down to the little brick rowhouses) instantly swaps to a
size-scaled rubble heap. Fly straight into a small brick building or storefront
and it skips the drama and just **collapses to rubble on impact**.

The building models live in `citybuildings.js` (`CITY_BUILDINGS`, pushed into
`MODEL_CATALOGS`); placements come from `tools/citygen/placebuildings.js`
(frontage-walks the downtown box, front = model +X, tiers by distance from the
skyscraper cluster) baked into `REMAP_MODELS` / `westchase_map.json`.

...
- mrfzstns (-262,103) FEATURE: cars driving into the lake should sink + stall (currently blocked/frozen at water edge?) — FIXED@v1.67.7 (see FUN-POLISH ROUND 1)
...
- mrg4iels (-153,22) quest NPCs should FACE the player when near (design ask, small) — FIXED@v1.67.9 (updateQActors eases rotation.y to face the player within 7u, back to authored home yaw when they leave; verified vivian 2.40->0.79->2.40)
...
- mrg52jt3 (-88,16) FEATURE: punch blood should be a cool sprite + balloons need string physics — FIXED@v1.67.7 (see FUN-POLISH ROUND 1)
...
## FUN-POLISH ROUND 1 — SHIPPED@v1.67.7 (funpolish, fable). Owner standing directive: "keep adding fun details".
- mrfzstns (-262,103) cars should sink+stall in the lake (currently blocked at shore) — FIXED@v1.67.7. The lake collider (.lake-flagged) still walls off NPC/traffic cars; only the PLAYER-DRIVEN car now uses the lake-filtered collider list (landColliders, same one on-foot wading uses) in updateDriving's pushOut, so it can cross the shoreline (the fountain + every non-lake collider still block it). New lakeCarPhysics(c,dt): once the driven car sits in inLake() water with lakeBedY < -0.6 it FLOODS — engine cut (stopEngine + engineTick skipped), throttle authority zeroed (accel=0 when c.flooding/c.sunk), heavy water drag decays pspeed→0, and the body sinks toward bedY+0.15 over ~1.5-2.5s with a random pitch/roll tilt + rising bubble puffs + 'splash' sfx + "ENGINE FLOODED — GET OUT" prompt (weaponBox). When settled → c.sunk (dead husk): re-entry blocked (enterCar/startBreakIn guard c.sunk), and exitCar drops the player INTO the water (lake-filtered pushOut) so the swim/underwater system takes over. Host tows the husk ~30s later via the standard explode/respawn machinery (updateCars stolen-branch). Works while driving for BOTH host and pure CLIENT (local driver owns its car pos; applyWorldSnap skips c===driving so the sink isn't yanked to y=0). MP CAVEAT (honest): the host doesn't model the lake — after a client bails, host authority resurfaces/redrives the car for other peers; full MP sink would need host-side lake logic (deferred, low value). Verified headless: car floods at bedY-1.34, pspeed 0.54→0.02→0, y sinks -0.73→-1.12, sunk=true, exit lands in lake, re-entry blocked, normal driving elsewhere maxSpeed 26 / y=0, traffic pushOut still ejects lake-interior points 48u. Evidence: funpolish_lake_1_approach/2_sinking/3_sunk.png, funpolish_normal_drive.png.
- mrg52jt3 (-88,16) balloon prop needs string physics when it moves + punch should spawn a cool blood sprite — FIXED@v1.67.7. (1) BALLOON: the held-balloon string (attachAccessory 'balloon', updateHandAcc) was a single rigid vertical cylinder; replaced with a 3-segment verlet rope (updateBalloonString) simulated in world space — bottom point pinned to the hand bone, top point (balloon) pulled UP by buoyancy (26 u/s², 10 for mid nodes) with air drag (0.9 velocity retention) + 3 constraint iterations holding each 0.22u link taut. The balloon body child follows the rope top and each segment mesh bridges two nodes, so the balloon floats ~straight up at rest and swings/lags/trails on motion. Only simulated within ~70m of the camera (snaps straight when far). Verified: rest horizontal offset ~0.24, motion jerk spikes to ~0.97 (clear trailing lag). Evidence: funpolish_balloon_1_rest.png (floats up), funpolish_balloon_2_motion.png (trails behind on a stretched string). (2) PUNCH: fist-melee hits now call bloodPunch() instead of a single blood puff — double the standard droplet spray + 4 faster fling droplets + a brief additive 2D impact star sprite (getImpactStar canvas, 'spark' flag in puffs/updatePuffs) at the contact point (target - forward*0.45, chest height). Reuses the existing puff system, no new asset pipeline. Applied to NPC / cop / mirrored-cop / remote-player melee connects. Evidence: funpolish_punch_blood.png (big red spray + white impact star).

## Live batch 8 (Alex on v1.67.9)
- mrgms310 (12,197) AK looks twisted + rate of fire too slow, "ground it in the CS:GO AK" — OPEN (fp-arms round owner: AK viewmodel twist + bump AK fire rate toward CS:GO ~600rpm feel)
- mrgmsvpb arm clipping through body — IN-AGENT (worldpolish2, fable)
- mrgmtcce reversed sign — IN-AGENT (worldpolish2, fable)
- mrgmu0ai floating coconuts — IN-AGENT (worldpolish2, fable)
...
- mrgn7702 floating prop / mrgn7ixv parking-line z-fight — IN-AGENT (worldpolish2, fable)

## WORLD-POLISH ROUND 2 (worldpolish2) — v1.67.12
- mrgmu0ai (-23,87) floating coconuts — FIXED@v1.67.12. Root cause: palms had NO coconut mesh at all, and the frond crown is rigidly parented to the trunk top (crownMesh.position.y=h), so nothing could physically detach/float — the report describes coconuts that didn't exist. Added a real drupe cluster (PALM_COCOS: 5 merged brown spheres, ONE shared geometry, cocoMat 0x6b4a2b) seated at the crown base (y=h-0.2) as a CHILD of the palm group, so it leans/spins with the trunk and can NEVER float off. ~55% of palms fruit. Map-wide via palm(). Verified: 55/96 palms carry the cluster, cocoY = trunk_top-0.2, renders tucked under the crown (wp2_cocoverify_after.png, visible in wp2_park_a_after.png).
- mrgn7ixv (-82,5) parking-line z-fight — FIXED@v1.67.12. Root cause: overlapping parking-lot surfaces sit at ~y=0.1 with NO polygonOffset and only a 0.0004 y-ladder — too little depth separation, so coplanar lots (remap Publix lot + merged house-lot apron + legacy parkingLot planes) z-fight; the stripes are baked INTO the parkingT texture, so the whole lot flickers in/out with camera angle. Fix: shared parkDepthBias() stamps a per-surface polygonOffset (factor -1, units -1-(seq%14)) on every parking material across all three builders (parkingLot(), remapSurfaces() parking+pavement, merged HOUSE_LOTS apron), giving each overlapping lot a distinct camera-independent depth like the road-paint pattern. No visible y change. Verified by geometry (surface stack at -82,5 now depth-separated) + clean render (wp2_park_a_after.png crisp stripes, no mottling); swiftshader static capture can't reproduce flicker directly (per coordinator guidance).
